...
> So when you want to make a query faster and you see that its explain plan is lack- ing index support, think about the query in terms of SLA in your application. Does this query need to run as fast as possible, even when it means that you now have to maintain more indexes?
